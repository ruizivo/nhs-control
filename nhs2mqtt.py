...
class NHS:
    ...
    def descobrePadrao(self, line):
        # using List comprehension + Brute Force 
        # Check if string repeats itself 
        # line2 = ''.join(line).decode('utf-8')
        stringlist=[x.decode('unicode_escape').encode('utf-8') for x in line]
        res = None
        for i in range(1, len(line)//2 + 1): 
            if (not len(line) % len(line[0:i]) and line[0:i] *
                (len(line)//len(line[0:i])) == line): 
                res = line[0:i] 
